...
int RoboteqDevice::SetCanCommand(int canIndex, int commandItem, int index, int value)
{
	string response;
	string head;
	char command[10];
	...
	int status = IssueCommand(head, command, args, 1, response, true);
	if(status != RQ_SUCCESS)
		return status;
	if(response != "+")
...
int RoboteqDevice::GetCanValue(int canIndex, int operatingItem, int index, int &result)
{
	string response;
	string head;
	char command[10];
	...
	int status = IssueCommand(head, command, args, 0, response);
	if(status != RQ_SUCCESS)
		return status;
...
int RoboteqDevice::Connect(string port)
{
	...
	if (status != RQ_SUCCESS)
	{
		...
		Disconnect();
		return RQ_UNRECOGNIZED_DEVICE;Issue
	}
