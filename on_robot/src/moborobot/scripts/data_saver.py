...
def left_camera_callback(ros_image):
    ...
    file_name = "image-"+timestr+".png"  # Generate file name according to time
...
def depth_callback(depth_image):
    ...
    file_name = "depth_image-"+timestr+".png"
...
def lidar_callback(ros_cloud):
    ...
    timestr = datetime.now().strftime(
        "%Y_%m_%d-%H_%M_%S_%f")[:-3]  # Take current time
    file_name = "lidar-"+timestr+".pcd"     # Generate file name according to time
